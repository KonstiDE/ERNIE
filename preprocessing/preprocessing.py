...
linebreak_pattern = re.compile(r'(?<=[a-z])\r?\n')
...
def clean(
    ...
    if emojis:
        df["main_content"] = [re.sub(emoji_pattern, " ", text) for text in df["main_content"]]
    ...
    if stopwords:
        ...
        with tqdm(df["main_content"], total=num_docs) as t:
            for doc_text in t:
                lang = str(
                    model.predict(doc_text.replace("\n", " "))[0][0]
                ).split("__")[2].split("_")[0]

                complete_doc = []

                if lang == "jpn" or lang == "zh" or lang == "yue":
                    ...
                    complete_doc.extend(doc_text.split(" "))
    ...
    if urls:
        df["main_content"] = [re.sub(url_pattern, "", text) for text in df["main_content"]]
    ...
    if hashtags:
        if hashtags_content:
            df["main_content"] = [re.sub(hashtag_content_pattern, "", text) for text in df["main_content"]]
        else:
            df["main_content"] = [re.sub(hashtag_pattern, "", text) for text in df["main_content"]]
    ...
    if ats:
        if ats_content:
            df["main_content"] = [re.sub(at_content_pattern, "", text) for text in df["main_content"]]
        else:
            df["main_content"] = [re.sub(at_pattern, "", text) for text in df["main_content"]]
    ...
    if punctuation:
        df["main_content"] = [re.sub(punct_pattern, "", text) for text in df["main_content"]]
        print("(7/10) Removed punctuation")
    else:
        print("(7/10) Skipped punctuation")

    if digits:
        df["main_content"] = [re.sub(digit_pattern, "", text) for text in df["main_content"]]
    ...
    df = df[df["main_content"] != " "]
    df["main_content"] = [re.sub(linebreak_pattern, " ", text) for text in df["main_content"]]

    print("(9/10) Removed empties and linebreaks (default)")
