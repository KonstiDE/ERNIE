...
def clean(
    ...
    if stopwords:
        ...
        df["cleaned_main_content"] = documents_cleaned
    ...
    df = df[df["cleaned_main_content"] != ""]
    print("(9/10) Removed empties (default)")

    if duplicate_cleanup:
        df = df.drop_duplicates(subset="cleaned_main_content")
...
def save_preprocess(df):
    df = df[df["cleaned_main_content"].notna()]
...
def save_preprocessed_as_text(df):
    ...
    with open("preprocessed.txt", "w+") as f:
        for cleaned_line in df["cleaned_main_content"].values.tolist():
